...
double sfunc(double L1, double L2)
{
    return log(1+exp(-abs(L1+L2)))-log(1+exp(-abs(L1-L2)));
}
        ...
        for(j=0;j<c1;j++)
        {
            ...
            for(k=0;k<c1;k++)
            {
                if(k != j)
                {
                    ...
                    {
                        Ltemp = max(0,Ltemp+e[cnode[i][k]].q) - max(Ltemp,e[cnode[i][k]].q) + sfunc(Ltemp, e[cnode[i][k]].q);
                    }
                }
            }
            e[cnode[i][j]].r = Ltemp;
